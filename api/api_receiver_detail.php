...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    foreach ($array as $row) {
        $Receiver_ID = $row['myReceiverID'];
        $Drug_ID = $row['myDrugID'];
        $Amount = $row['myAmount'];
        $Net_Price = $row['myNetPrice'];

        $sql .= "INSERT INTO receiver_detail (Receiver_ID, Drug_ID, Amount, Net_Price) 
        VALUES ('$Receiver_ID', '$Drug_ID', '$Amount', '$Net_Price');"; // Make Multiple Insert Query 
        $arr_data = array();
        $jsonRow = array(
