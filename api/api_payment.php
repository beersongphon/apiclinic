...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    //Add user{
    if (isset($request['myActionType'])) {
        $Emp_ID = $request['myEmpID'];

        if ($request['myActionType'] == 'A') {
            $Pay_ID  = $request['myPayID'];
            $Remedy_ID = $request['myRemedyID'];
            $Patient_ID = $request['myPatientID'];
            $Total_Price  = $request['myTotalPrice'];

            $sql = "INSERT INTO payments_patient (Pay_ID, Remedy_ID, Patient_ID, Emp_ID, Total_Price) VALUES ('$Pay_ID', '$Remedy_ID', '$Patient_ID', '$Emp_ID', '$Total_Price')";
            $result = $conn->query($sql);
            if ($result) {
                $sql = "UPDATE dispense SET Status = 'ชำระเงินเรียบร้อย' WHERE Remedy_ID = '$Remedy_ID'";
                $result = $conn->query($sql);
                if ($result) {
        ...
        } elseif ($request['myActionType'] == 'B') {
            $PayDealer_ID  = $request['myPayDealerID'];
            $Dealer_ID = $request['myDealerID'];
            $Receiver_ID = $request['myReceiverID'];
            $Total_Amount  = $request['myTotalAmount'];

            $sql = "INSERT INTO payments_dealer (PayDealer_ID, Dealer_ID, Emp_ID, Receiver_ID, Total_Amount) VALUES ('$PayDealer_ID', '$Dealer_ID', '$Emp_ID', '$Receiver_ID', '$Total_Amount')";
            $result = $conn->query($sql);
            if ($result) {
                $output['status'] = "success";
                $output['message'] = "บันทึกข้อมูลสำเร็จ";
    ...
    elseif (isset($request['txtSearch'])) {
        ...
        (SELECT DISTINCT prescription.Prescription_ID FROM prescription WHERE prescription.Remedy_ID = remedy.Remedy_ID LIMIT 1) AS Prescription_ID,
        (SELECT DISTINCT (disease.Disease_Price+dispense.Total_Amount) FROM dispense WHERE dispense.Prescription_ID = Prescription_ID LIMIT 1) AS Total_Price, 
        (SELECT DISTINCT (dispense.Dispense_ID) FROM dispense WHERE dispense.Prescription_ID = Prescription_ID LIMIT 1) AS Dispense_ID 
        FROM appoint 
        LEFT JOIN appoint_detail ON appoint_detail.App_ID = appoint.App_ID
        LEFT JOIN remedy ON remedy.Remedy_ID = appoint.Remedy_ID
        LEFT JOIN remedy_detail ON remedy_detail.Remedy_ID = remedy.Remedy_ID
        ...
        LEFT JOIN dispense ON dispense.Patient_ID = appoint.Patient_ID
        WHERE dispense.Status = 'รอการชำระเงิน'";
        // $sql .= "SELECT * FROM patient";

        $txtSearch = $request['txtSearch'];

        if (strlen($txtSearch) > 0) {
            $sql .= " AND patient.Patient_ID = '$txtSearch'
