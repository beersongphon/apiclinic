...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    //Add user{
    if (isset($request['myAmortID'])) {
        $Amort_ID  = $request['myAmortID'];
        $Emp_ID  = $request['myEmpID'];

        $sql = "INSERT INTO amort (Amort_ID, Emp_ID) VALUES ('$Amort_ID', '$Emp_ID')";
    ...
    elseif (isset($request['txtSearch'])) {
        $sql = "SELECT * 
        FROM amort
        LEFT JOIN employee ON employee.Emp_ID = amort.Emp_ID";
        // $sql .= "SELECT * FROM patient";

        $txtSearch = $request['txtSearch'];

        if (strlen($txtSearch) > 0) {
            $sql .= " WHERE amort.Amort_ID = '$txtSearch'
