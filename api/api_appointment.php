...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    elseif (isset($request['prescriptionid'])) {
        $sql = "SELECT *, 
        ...
        LEFT JOIN employee ON employee.Emp_ID = prescription.Emp_ID
        LEFT JOIN patient ON patient.Patient_ID = prescription.Patient_ID
        LEFT JOIN card_patient ON card_patient.Patient_ID = patient.Patient_ID";

        $Prescription_ID = $request['prescriptionid'];

        if (strlen($Prescription_ID) > 0) {
            $sql .= " WHERE prescription.Prescription_ID = '$Prescription_ID'";
        }
        $result = $conn->query($sql);
        $arr = array();
